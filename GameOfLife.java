	...
	public static void test2(String fileName) {
		...
		for (int i = 1; i < board.length-1; i++)
		{
			for (int j = 1; j < board[0].length-1; j++) {
				if (board[i][j] != cellValue(board, i, j)) {
					System.out.println("There will be a change in ["+i+"]["+j+"]. There are " + count(board, i, j) + " alive cells around. Therefore, in the next generation board ["+i+"]["+j+"] = " + cellValue(board, i, j));
				}
			}
		}
